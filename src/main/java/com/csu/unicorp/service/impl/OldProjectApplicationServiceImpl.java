...
// import com.csu.unicorp.mapper.OldProjectApplicationMapper;
// import com.csu.unicorp.mapper.OldProjectMapper;
// import com.csu.unicorp.mapper.OldProjectMemberMapper;
...
// public class OldProjectApplicationServiceImpl extends ServiceImpl<OldProjectApplicationMapper, OldProjectApplication> implements OldProjectApplicationService {

//     private final OldProjectMapper projectMapper;
//     private final OldProjectApplicationMapper projectApplicationMapper;
//     private final OldProjectMemberMapper projectMemberMapper;
//     private final UserMapper userMapper;
//     private final OldProjectService projectService;
//     private final OrganizationMapper organizationMapper;
//     private static final Logger log = LoggerFactory.getLogger(OldProjectApplicationServiceImpl.class);
...
//     @Override
//     @Transactional
//     public OldProjectApplicationDetailVO applyForProject(Integer projectId, OldProjectApplicationCreationDTO dto, UserDetails userDetails) {
//         // 获取当前用户
...
//         // 1. 查是否有未删除的申请
//         LambdaQueryWrapper<OldProjectApplication> queryWrapper = new LambdaQueryWrapper<>();
//         queryWrapper.eq(OldProjectApplication::getProjectId, projectId)
//                 .eq(OldProjectApplication::getUserId, currentUser.getId())
//                 .eq(OldProjectApplication::getIsDeleted, false);

//         OldProjectApplication existingApplication = projectApplicationMapper.selectOne(queryWrapper);
...
//         // 2. 2. 查询当前已批准成员数
//         LambdaQueryWrapper<OldProjectApplication> memberCountWrapper = new LambdaQueryWrapper<>();
//         memberCountWrapper.eq(OldProjectApplication::getProjectId, projectId)
//                           .eq(OldProjectApplication::getStatus, "approved")
//                           .eq(OldProjectApplication::getIsDeleted, false);
...
//         // 4. 没有则插入新记录
//         OldProjectApplication application = new OldProjectApplication();
...
//      */
//     @Override
//     public List<OldProjectApplicationDetailVO> getProjectApplications(Integer projectId, UserDetails userDetails) {
//         // 获取当前用户
//         User currentUser = userMapper.findByUsername(userDetails.getUsername());
//         if (currentUser == null) {
//             throw new AccessDeniedException("用户不存在");
...
//     @Override
//     @Transactional
//     public OldProjectApplicationDetailVO updateApplicationStatus(Integer applicationId, OldProjectApplicationStatusUpdateDTO dto, UserDetails userDetails) {
//         // 获取当前用户
//         User currentUser = userMapper.findByUsername(userDetails.getUsername());
//         if (currentUser == null) {
//             throw new AccessDeniedException("用户不存在");
...
//         // 检查申请是否存在
//         OldProjectApplication application = projectApplicationMapper.selectById(applicationId);
...
//             LambdaQueryWrapper<OldProjectMember> memberQuery = new LambdaQueryWrapper<>();
//             memberQuery.eq(OldProjectMember::getProjectId, project.getId())
//                        .eq(OldProjectMember::getUserId, currentUser.getId())
//                        .eq(OldProjectMember::getIsDeleted, false);
//             long count = projectMemberMapper.selectCount(memberQuery);
//             if (count == 0) {
//                 OldProjectMember member = new OldProjectMember();
...
//     ) {
//         // 1. 查出该学生所有申请记录
//         List<OldProjectApplication> applications = projectApplicationMapper.selectList(
//             new LambdaQueryWrapper<OldProjectApplication>()
//                 .eq(OldProjectApplication::getUserId, userId)
//                 .eq(OldProjectApplication::getIsDeleted, false)
//         );
...
//         }
//         // 2. 拿到所有申请过的项目ID
//         List<Integer> projectIds = applications.stream().map(OldProjectApplication::getProjectId).collect(Collectors.toList());
...
//             }
//             // 申请相关字段
//             OldProjectApplication app = applications.stream()
...
//      */
//     private OldProjectApplicationDetailVO convertMapToDetailVO(Map<String, Object> map) {
//         OldProjectApplicationDetailVO vo = new OldProjectApplicationDetailVO();
...
//         OldProjectApplicationDetailVO.ApplicantProfileVO profileVO = new OldProjectApplicationDetailVO.ApplicantProfileVO();
...
//      * 将实体转换为VO
//      */
//     private OldProjectApplicationDetailVO convertToDetailVO(OldProjectApplication application, User applicant) {
//         OldProjectApplicationDetailVO vo = new OldProjectApplicationDetailVO();
