...
public ProjectVO updateProject(ProjectCreationDTO dto) {
    ...
    public PageResultVO<ProjectVO> getMyProjectList(String status, String initiatorType, String field, String keyword, Integer initiatorId, Integer organizationId, int page, int pageSize,Integer userId) {
...
public ProjectVO getProjectById(Integer projectId) {
    ...
    BeanUtils.copyProperties(project, vo);
    if (project.getAttachments() != null) {
        List<String> storedNames = List.of(project.getAttachments().split(","));
        ...
        if (!storedNames.isEmpty()) {
            ...
            List<String> originalNames = storedNames.stream()
                .map(stored -> mappings.stream()
                    ...
                    .findFirst().orElse(stored)) // 查不到就用存储名兜底
                .collect(Collectors.toList());
...
public PageResultVO<ProjectVO> getProjectList(String status, String initiatorType, String field, String keyword, Integer initiatorId, Integer organizationId, int page, int pageSize,Integer userId) {
    ...
    if (organizationId != null) {
        ...
        List<Integer> userIds = users.stream().map(User::getId).collect(Collectors.toList());
        if (!userIds.isEmpty()) {
            wrapper.notIn("initiator_id", userIds);
        } else {
            // 没有用户，直接返回空分页
            return new PageResultVO<>(0, Collections.emptyList());
        }
